      ...
      <header className={cn(
        ...
        <div className="container mx-auto px-4 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {["Pradžia", "Apie", "Analizė"].map((item) => (
              <Link key={item} href={`#${item.toLowerCase()}`} className="px-3 py-2 text-sm font-medium text-text-medium hover:text-white relative group">
                {item}
            ...
            <div className="p-4">
              ...
              <nav className="space-y-4">
                {["Pradžia", "Apie", "Analizė", "Reikšmės", "DUK"].map((item) => (
                  <Link key={item} href={`#${item.toLowerCase()}`} className="block px-4 py-3 text-text-medium hover:text-white transition-colors">
                    {item}
                  </Link>
                ))}
              </nav>
